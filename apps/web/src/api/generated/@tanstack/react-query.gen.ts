 ...
 * Duplicate load test
 *
 * Create a copy of an existing load test in `draft` status with the same configuration. Run history is not copied.
 */
export const loadTestsDuplicateMutation = (options?: Partial<Options<LoadTestsDuplicateData>>): UseMutationOptions<LoadTestsDuplicateResponse, DefaultError, Options<LoadTestsDuplicateData>> => {
    const mutationOptions: UseMutationOptions<LoadTestsDuplicateResponse, DefaultError, Options<LoadTestsDuplicateData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await loadTestsDuplicate({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
